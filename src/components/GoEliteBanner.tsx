...
const GoEliteBanner = ({ active, onToggle }: Props) => (
  <button
    type="button"
    role="switch"
    aria-checked={active}
    aria-label="Toggle Go Elite filter"
    onClick={onToggle}
    className={`shrink-0 whitespace-nowrap h-8 pl-1.5 pr-3 rounded-xl text-xs font-semibold inline-flex items-center gap-2 border transition-colors ${
      active
        ? "bg-gradient-to-r from-yellow-500 to-amber-600 text-white border-transparent"
